...
# BASE_DIR = "simulation/combinational-circuit/partitioning1"
...
BASE_DIR = "simulation/combinational-circuit/partitioning2"
FILE_PATTERN = r"\.mt[a-zA-Z0-9]+$"
OUTPUT_JSON_NAME = "measure.json"
...
def build_delay_summary(output_data):
    ...
    for test in output_data.values():
        if not isinstance(test, dict):
            continue
        for name, val in test.get("delay", {}).items():
            # For summary stats, we MUST ignore None (Failed) values
            if val is None:
                continue
            delay_values.setdefault(name, []).append(val)
        for name, val in test.get("measurements", {}).items():
    ...
    for name, values in delay_values.items():
        ...
        if len(parts) >= 3:
            dir_type = parts[0]      # tplh or tphl
            out_sig = parts[1]       # output signal or block name
            if out_sig not in output_dirs:
                output_dirs[out_sig] = {"tplh": [], "tphl": []}
            if dir_type in ("tplh", "tphl"):
                output_dirs[out_sig][dir_type].extend(values)
        ...
        if "_clba_" in name_lower:
            block_delays["clba"].extend(values)
        elif "_clbb_" in name_lower:
            block_delays["clbb"].extend(values)
        elif name_lower.startswith("tplh_out") or name_lower.startswith("tphl_out"):
            block_delays["out"].extend(values)
    ...
    for block, vals in block_delays.items():
        if vals:
            summary[f"worst_case_delay_{block}"] = max(vals)
            summary[f"best_case_delay_{block}"] = min(vals)
            summary[f"avg_case_delay_{block}"] = sum(vals) / len(vals)
...
def process_delay_records(records, signals):
    ...
    for rec_idx, rec in enumerate(records):
        ...
        for key in meas_headers:
            ...
            if is_delay:
                # We KEEP all values including None (Failed) as requested.
                # We no longer restrict delays to only those matching the primary transition signal.
                delay[key] = val
            else:
                if not isinstance(val, float):
                    continue
